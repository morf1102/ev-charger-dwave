...
def read_in_args():
    ...
    parser.add_argument(
        "-x", "--width", help="set the width of the grid", default=15, type=int
    )
    parser.add_argument(
        "-y", "--height", help="set the height of the grid", default=15, type=int
    )
    parser.add_argument(
        "-p", "--poi", help="set the number of POIs", default=3, type=int
    )
    parser.add_argument(
        "-c",
        "--chargers",
        help="set the number of existing chargers",
        default=4,
        type=int,
    )
    parser.add_argument(
        "-n",
        "--new-chargers",
        help="set the number of new chargers",
        default=2,
        type=int,
    )
...
def build_bqm(
    potential_new_cs_nodes, num_poi, pois, num_cs, charging_stations, num_new_cs
):
...
def run_bqm_and_collect_solutions(bqm, sampler, potential_new_cs_nodes, **kwargs):
    ...
    sampleset = sampler.sample(
        bqm, num_reads=10, label="Example - EV Charger Placement", **kwargs
    )
...
def printout_solution_to_cmdline(
    pois, num_poi, charging_stations, num_cs, new_charging_nodes, num_new_cs
):
    ...
    if num_cs > 0:
        old_cs_avg_dist = [
            sum(abs(a - b) for a, b in zip(new, loc) for loc in charging_stations)
            / num_cs
            for new in new_charging_nodes
        ]
    ...
    if num_new_cs > 1:
        ...
        for i in range(num_new_cs):
            for j in range(i + 1, num_new_cs):
                new_cs_dist += abs(
                    new_charging_nodes[i][0] - new_charging_nodes[j][0]
                ) + abs(new_charging_nodes[i][1] - new_charging_nodes[j][1])
...
if __name__ == "__main__":
    ...
    new_charging_nodes = run_bqm_and_collect_solutions(
        bqm, sampler, potential_new_cs_nodes
    )

    # Print results to commnand-line for user
    printout_solution_to_cmdline(
        pois,
        args.poi,
        charging_stations,
        args.chargers,
